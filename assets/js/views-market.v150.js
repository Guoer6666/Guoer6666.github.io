/* ============================================================
   ...
   ============================================================ */
const V = {};   // 全局视图注册表（views-market2 等继续挂载）
...
V.usnight = {
  ...
  async mount(reg) {
    ...
    const paintUsSectors = () => {
      ...
      if (!el) return;
      if (!sec || !sec.ok) {
        el.innerHTML = '<div class="flash-empty">美股板块榜暂无可靠数据源：原东方财富板块接口已下线。宁缺勿假，不再用 ETF 涨跌幅代理（与真实板块榜口径有偏差）；待找到真实板块数据源后本卡自动恢复。</div>';
