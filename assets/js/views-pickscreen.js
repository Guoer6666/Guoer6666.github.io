...
V.pickscreen = (() => {
  ...
  function ledgerHtml(todayList, ntdLabel, todayDate) {
    ...
    /* ① 速览：累计战绩（轻量一行；完整明细在「实盘每日记录」，月/季/年在「周期总结」） */
    if (cum.n_total) {
    ...
    /* 分工指路（2026-09-08 去重：持仓/历史/汇总不再在本页重复展示） */
    h += '<div class="lg-foot" style="margin:4px 0 10px">📍 本页只管<b>今天怎么操作</b>：名单 + 规则 + 明日执行计划。'
      + '昨日执行明细与历史战绩 → <b>「实盘每日记录」</b>；月/季/年汇总与归档 → <b>「周期总结」</b>。</div>';
    ...
    /* ③ 昨天选出 → 今天买入（持仓中）：已移至「实盘每日记录」页，本页不再重复展示 */
    ...
    /* ⑥ 历史已了结：已移至「实盘每日记录」（批次明细）与「周期总结」（按月归档汇总），本页不再重复 */
  ...
  /* ---------------- 中期体检（复用「个股逻辑拆解」九模块引擎，实时真实计算） ----------------
     为什么需要：选股引擎只实现了解拆九模块中的技术动量 5 模块（趋势/量能/α/位置/风险），
     估值分位、财报扣非、解禁减持、股东户数从未参与选股打分。因此一只票完全可能
     「选股满分入选」同时「中期体检偏空」。此处把中期体检结论显式化，避免用户两边手工对照。
     铁律：只展示引擎真实返回值；拉不到/报错显示「无法验证」，绝不显示编造分数。
     结果按 代码+选股日期+体检日 缓存，同一天不重复消耗接口。 */
  const CK_STORE = 'pick_checkup_cache';
  const CK_RUN = {};                                  // 运行中标记（内存态，不持久化）
  let ckSeq = 0;                                      // 串行执行序号，防并发打爆接口

  function ckCache() { return Store.get(CK_STORE, {}) || {}; }
  function ckKey(code, pickDate) {
    const d = DT ? DT.today() : new Date().toISOString().slice(0, 10);
    return String(code) + '_' + (pickDate || '') + '_' + d;
  }
  function ckGet(code, pickDate) { return ckCache()[ckKey(code, pickDate)] || null; }
  function ckPut(code, pickDate, obj) {
    const c = ckCache(); c[ckKey(code, pickDate)] = obj; Store.set(CK_STORE, c);
  }
  function ckClear(pickDate) {
    const c = ckCache();
    Object.keys(c).forEach(k => { if (!pickDate || k.indexOf('_' + pickDate + '_') >= 0) delete c[k]; });
    Store.set(CK_STORE, c);
  }

  async function ckRunOne(code, pickDate, name) {
    let hasAna = false;
    try { hasAna = !!(typeof Ana !== 'undefined' && Ana && typeof Ana.quickScore === 'function'); }
    catch (e) { hasAna = false; }
    if (!hasAna) {
      ckPut(code, pickDate, { st: 'error', msg: '拆解引擎未加载，无法体检' });
      return;
    }
    if (CK_RUN[code]) return;
    CK_RUN[code] = true;
    ckPut(code, pickDate, { st: 'running' });
    App.refresh();
    try {
      const r = await Ana.quickScore(code, pickDate, name);
      ckPut(code, pickDate, { st: 'done', r: r });
    } catch (e) {
      ckPut(code, pickDate, { st: 'error', msg: (e && e.message) ? e.message : String(e) });
    } finally {
      delete CK_RUN[code];
      App.refresh();
    }
  ...
  /* 一键体检：串行执行（并会打爆东财/腾讯接口），逐只刷新 */
  function ckRunAll(list, pickDate) {
    const my = ++ckSeq;
    const q = (list || []).filter(r => r && r.code && !CK_RUN[r.code]);
    if (!q.length) { UI.toast('没有需要体检的标的（已体检或正在体检）'); return; }
    UI.toast('开始体检 ' + q.length + ' 只（串行执行，实时真实数据，约需 ' + (q.length * 8) + ' 秒）');
    (async () => {
      for (let i = 0; i < q.length; i++) {
        if (my !== ckSeq) return;                     // 用户又点了一次，本轮作废
        await ckRunOne(q[i].code, pickDate, q[i].name);
      }
      if (my === ckSeq) UI.toast('✅ 中期体检完成（' + q.length + ' 只，全部为引擎实时计算）');
    })();
  }

  /* 单只体检结果的可视化：只呈现真实返回，缺失一律「无法验证」 */
  function ckChip(code, pickDate) {
    const c = ckGet(code, pickDate);
    if (!c) return '<button class="m26-ck-btn" onclick="V.pickscreen.act(\'ckone\',\'' + code + '\')">体检</button>';
    if (c.st === 'running') return '<span class="m26-ck-run">体检中…</span>';
    if (c.st === 'error') return '<span class="m26-ck-err" title="' + UI.esc(c.msg || '') + '">无法验证</span>';
    const r = c.r || {};
    const net = r.net;
    const bear = net != null && net < 0;
    const cls = bear ? 'm26-ck-bad' : (net != null && net >= 3 ? 'm26-ck-good' : 'm26-ck-mid');
    const sigTxt = (r.signal && r.signal.cn) ? r.signal.cn : '—';
    const conf = r.confidence != null ? r.confidence : '—';
    const tip = [('净分 ' + (net > 0 ? '+' : '') + net), ('评级 ' + (r.rating || '—')),
      ('可信度 ' + conf + '/100'), ('模块 ' + r.okMods + '/' + r.modTotal)]
      .concat(r.modErrs && r.modErrs.length ? ['失败模块：' + r.modErrs.join('；')] : [])
      .concat(r.topBear && r.topBear.length ? ['主要压制：' + r.topBear.join('；')] : [])
      .join('\n');
    return '<span class="m26-ck ' + cls + '" title="' + UI.esc(tip) + '">'
      + (bear ? '⚠️ ' : '') + UI.esc(sigTxt) + ' ' + (net > 0 ? '+' : '') + net
      + '</span>'
      + '<button class="m26-ck-btn" onclick="V.pickscreen.act(\'ckone\',\'' + code + '\')" title="重新体检">↻</button>';
  }
  ...
  function render() {
    ...
    if (res.dList && res.dList.length) {
      h += '<div class="m26-sec-t">🎯 D · ' + ntd.label + '（Top ' + res.dList.length + '，含逻辑拆解最终建议）</div>'
        + '<div class="m26-ck-bar">'
        + '<button class="m26-ck-all" onclick="V.pickscreen.act(\'ckall\')">🩺 一键中期体检（' + res.dList.length + ' 只）</button>'
        + '<button class="m26-ck-btn" onclick="V.pickscreen.act(\'ckclear\')">清空结果</button>'
        + '<span class="m26-cell-sub">复用「个股逻辑拆解」九模块引擎实时计算（含估值分位/财报/解禁减持/股东户数）；数据拉不到显示「无法验证」，不编造</span>'
        + '</div>'
      ...
      res.dList.forEach((r, i) => {
        h += '<tr class="m26-row-d' + (boardInfo(r.code).tradable ? '' : ' m26-row-untrade') + '">'
          ...
          + '<td>' + ckChip(r.code, res.date) + '</td>'
          + '</tr>'
          + '<tr><td colspan="13" style="text-align:left;background:rgba(0,0,0,0.03)"><div class="m26-cell-sub" style="white-space:normal;line-height:1.6">'
  ...
  function act(fn, arg) {
    if (fn === 'run') run();              // ⚡ 盘中实时试算（浏览器内全市场重算，非最终决策）
      ...
      const res = Store.get('pickscreen_last', null);
      if (!res || !res.dList || !res.dList.length) { UI.toast('暂无选股名单'); return; }
      const it = res.dList.filter(x => String(x.code) === String(arg))[0];
      ckRunOne(String(arg), res.date, it ? it.name : '');
    }
    else if (fn === 'ckall') {
      const res = Store.get('pickscreen_last', null);
      if (!res || !res.dList || !res.dList.length) { UI.toast('暂无选股名单'); return; }
      ckRunAll(res.dList, res.date);
    }
    else if (fn === 'ckclear') {
      const res = Store.get('pickscreen_last', null);
      ckClear(res ? res.date : '');
      UI.toast('已清空本页中期体检结果（下次点击可重新计算）');
      App.refresh();
    }
