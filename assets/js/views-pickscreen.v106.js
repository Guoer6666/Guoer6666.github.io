...
V.pickscreen = (() => {
  ...
  function ledgerHtml(todayList, ntdLabel, todayDate) {
    ...
    /* ① 速览：累计战绩（轻量一行；完整明细在「选股次日验证」，月/季/年在「周期总结」） */
    if (cum.n_total) {
    ...
    h += '<div class="lg-foot" style="margin:4px 0 10px">📍 本页只管<b>今天怎么操作</b>：名单 + 规则 + 明日执行计划。'
      + '昨日执行明细与历史战绩 → <b>「选股次日验证」</b>；月/季/年汇总与归档 → <b>「周期总结」</b>。</div>';
    ...
    /* ③ 昨天选出 → 今天买入（持仓中）：已移至「选股次日验证」页，本页不再重复展示 */
    ...
    /* ⑥ 历史已了结：已移至「选股次日验证」（批次明细）与「周期总结」（按月归档汇总），本页不再重复 */
