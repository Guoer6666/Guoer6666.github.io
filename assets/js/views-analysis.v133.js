...
const Ana = (() => {
  ...
  /* ---------- 轻量体检（供「个股拆解选股」名单复用：同口径、不写任务历史、不占运行锁） ----------
     背景：选股引擎只实现了本引擎九模块中的技术动量 5 模块，估值分位/财报扣非/解禁减持/股东户数
     从未参与选股打分，导致「选股给买入、拆解给减持」的口径分歧。本函数在名单侧补跑完整九模块，
     把中期体检结论显式化，让冲突一眼可见，而不是让用户自己两边对照。
     铁律：全程复用本引擎真实数据源与真实规则；拉不到或报错一律返回 error，绝不返回编造分数。 */
  async function quickScore(code, date, name) {
    const t = {
      _id: 'qs_' + Date.now() + '_' + Math.random().toString(36).slice(2, 7),
      code: String(code || '').trim(), name: name || '', date: date || DT.today(),
      cfg: cfg(), done: {}
    };
    const base = await buildBase(t);
    ...
    await Promise.all(ids.map(async mid => {
      try {
        t.done[mid] = Object.assign({ status: 'done', at: DT.stamp() }, await loaders[mid](base, t));
      } catch (e) {
        modErrs.push(mid + '：' + (e && e.message ? e.message : String(e)));
        t.done[mid] = { status: 'error', at: DT.stamp(), points: [], signals: [] };
      }
    }));
    try { t.done.debate = Object.assign({ status: 'done', at: DT.stamp() }, modDebate(base, t)); }
    catch (e) { t.done.debate = { status: 'error', at: DT.stamp(), flaws: [], dialog: [] }; }
    const risk = modRisk(base, t);
    const okMods = ids.filter(k => t.done[k] && t.done[k].status === 'done').length;
    const val = (base.deep && base.deep.val) ? base.deep.val : {};
    return {
      code: t.code, name: (base.r && base.r.name) || t.name || '',
      date: t.date, at: DT.stamp(),
      net: risk.net, bullW: risk.bullW, bearW: risk.bearW,
      rating: risk.rating, ratingCls: risk.ratingCls,
      signal: risk.signal, confidence: risk.confidence, posTxt: risk.posTxt,
      pe: val.pe != null ? val.pe : null, pePct: val.pePct != null ? val.pePct : null,
      pb: val.pb != null ? val.pb : null, pbPct: val.pbPct != null ? val.pbPct : null,
      okMods: okMods, modTotal: ids.length, modErrs: modErrs,
      topBull: (risk.bull || []).slice(0, 2).map(s2 => s2.text),
      topBear: (risk.bear || []).slice(0, 2).map(s2 => s2.text)
    };
  }
  ...
  return { MODULES, tasks, getTask, start, pause, resume, stop, del, rerun, cfg, resolveStock, quickScore, runningId: () => _running };
})();
