...
(function () {
  ...
  function injectCss() {
    ...
    st.textContent = [
      ...
      '.sm-badge.listed{background:rgba(139,92,246,.15);color:#7c4dff}',
  ...
  function batchHtml(b) {
    ...
    else if (phase === 'listed') { badgeCls = 'listed'; badgeTxt = '名单已出·待验证'; }
    ...
    if (phase === 'empty') {
      ...
      h.push('<div class="sm-batch-s">'
        + '入选 <b>' + num(s.pick_n) + '</b> 只　'
        + esc(b.await_label || '等待下一交易日开盘验证（开盘价 ≤ 介入上限才买入，高开即放弃）')
        + '<br>开盘/收盘与收益待 T+1 验证跑批后自动填充'
        + '</div>');
    } else {
